...
// ── Upload document to Supabase Storage (admin only) ─────────────────────────
app.post('/api/admin/upload-document', upload.single('document'), requireAdmin, async (req, res) => {
    const { listingId, docType } = req.body;
    if (!listingId || !req.file) return res.status(400).json({ error: 'Missing file or listingId' });
    ...
    const fileName = `${listingId}/${docType || 'doc'}-${Date.now()}-${safeName}`;
    ...
    // Store document reference in listings.documents
    const { data: listing } = await supabase
        .from('listings').select('documents').eq('id', listingId).single();
    ...
    await logEvent('admin_upload_document', { listingId, docType, fileName });
    res.json({ success: true, fileName });
});

// ── OCR: read Aviso de Operación using Claude Vision ─────────────────────────
app.post('/api/admin/ocr-document', upload.single('document'), requireAdmin, async (req, res) => {
    if (!req.file) return res.status(400).json({ error: 'No file received' });
        ...
        const response = await axios.post('https://api.anthropic.com/v1/messages', {
            ...
            messages: [{
                ...
                content: [
                    ...
                    {
                        type: 'text',
                        text: `This is a Panamanian "Aviso de Operación" (business operating license) or related business document.
Please extract the following information and return ONLY a JSON object with these fields:
{
  "ruc": "the RUC number (tax ID, format like 1234567-1-234567 or similar)",
  "ruc_dv": "the DV (verification digit, usually 1-2 digits after the RUC)",
  "legal_name": "the legal business name or owner name",
  "license_number": "the aviso de operación number",
  "license_date": "the issue date in YYYY-MM-DD format if visible",
  "activity": "the business activity description if visible",
...
Return ONLY the JSON, no other text.`
        ...
        try {
            ...
            return res.status(500).json({ error: 'Could not parse OCR response', raw: content });
        }

        await logEvent('admin_ocr_document', { listingId: req.body.listingId, confidence: extracted.confidence });
...
// ── Save extracted OCR data to listing ───────────────────────────────────────
app.post('/api/admin/save-ocr-data', requireAdmin, async (req, res) => {
    const { id, ruc, ruc_dv, legal_name, license_number, license_date } = req.body;
    if (!id) return res.status(400).json({ error: 'Missing id' });

    const { error } = await supabase
        .from('listings')
        .update({
            ruc, ruc_dv, legal_name, license_number,
    ...
    if (error) return res.status(500).json({ error: error.message });
    await logEvent('admin_save_ocr_data', { id, ruc, legal_name });
    res.json({ success: true });
});

// ── Activate trial membership ─────────────────────────────────────────────────
app.post('/api/admin/activate-trial', requireAdmin, async (req, res) => {
    const { id } = req.body;
    if (!id) return res.status(400).json({ error: 'Missing id' });
    ...
    const { error } = await supabase
        ...
        .update({
            ...
            membership_paid_until: trialEnd.toISOString().split('T')[0]
        })
        .eq('id', id);

    if (error) return res.status(500).json({ error: error.message });
    await logEvent('trial_activated', { id, trial_end: trialEnd.toISOString().split('T')[0] });
    res.json({ success: true, trial_end: trialEnd.toISOString().split('T')[0] });
});

// ── Convert trial to full membership ─────────────────────────────────────────
app.post('/api/admin/convert-to-member', requireAdmin, async (req, res) => {
    const { id, months } = req.body;
    if (!id) return res.status(400).json({ error: 'Missing id' });

    const years = (months || 12) / 12;
    const paidUntil = new Date();
    paidUntil.setFullYear(paidUntil.getFullYear() + Math.floor(years));
    if (years % 1 !== 0) paidUntil.setMonth(paidUntil.getMonth() + Math.round((years % 1) * 12));
    ...
    const { error } = await supabase
        ...
        .update({
            is_member:             true,
            is_trial:              false,
            membership_paid_until: paidUntil.toISOString().split('T')[0]
        })
        .eq('id', id);

    if (error) return res.status(500).json({ error: error.message });
    await logEvent('trial_converted_to_member', { id, paid_until: paidUntil.toISOString().split('T')[0], months });
    res.json({ success: true, paid_until: paidUntil.toISOString().split('T')[0] });
});

// ── Get document signed URL (for admin to view) ───────────────────────────────
app.get('/api/admin/document-url', requireAdmin, async (req, res) => {
    const { path: filePath } = req.query;
    if (!filePath) return res.status(400).json({ error: 'Missing path' });
...
// ── Get members expiring soon (for reminders) ─────────────────────────────────
app.get('/api/admin/expiring-soon', requireAdmin, async (req, res) => {
    const days = parseInt(req.query.days) || 7;
    const today = new Date();
    ...
    const { data, error } = await supabase
        .from('listings')
        .select('id, name, email, phone, membership_paid_until, is_trial, contact_name')
        .eq('is_member', true)
        .gte('membership_paid_until', today.toISOString().split('T')[0])
        .lte('membership_paid_until', future.toISOString().split('T')[0])
        .order('membership_paid_until');
